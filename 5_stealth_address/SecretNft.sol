...
Ownable
{
    ...
    function mint(uint Qx, uint Qy, bytes calldata publishableData) public onlyOwner {
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        address _stealthRecipient = addressFromPoints(Qx, Qy);
        _safeMint(_stealthRecipient, tokenId);
        emit PrivateTransfer(_stealthRecipient, publishableData);
    }
